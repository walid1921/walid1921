...
**Frontend Development:** **`React`** **`React Query`** **`Rest-API's`** **`axios`** **`TypeScript`** **`Tailwind`** **`Sass`** **`Bootstrap`** **`Styled-components`** **`Framer-Motion`**
...
**Others:** **`Netlify`** **`Vercel`** **`Render`** **`Npm`** **`Yarn`** **`Scrum`** **`Jira`** **`Agile`**
